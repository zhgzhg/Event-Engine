...
public class DispatchersTest {
    ...
    void testEncapsulation() {
        ...
        sm = SerializationMechanism.JSON;
        cs = StandardCharsets.UTF_16;
        dp = new DataPacket(sm, cs, payload);

        packed = dp.toBytes();
        goodPacked = new byte[1 + SerializationMechanism.getCodeSize() + 1 + cs.name().length() + payload.length];
        goodPacked[0] = 0x00;
        goodPacked[1] = SerializationMechanism.JSON.getCode();
        ...
        assertThrows(IllegalArgumentException.class, () -> new DataPacket(SerializationMechanism.JAVA, null, null));
    }

    @Test
    void encapsulationWithImproperDataLengthTest() {
        byte badSize = 0x50;
        byte[] badPacket = new byte[]{0x00, SerializationMechanism.JAVA.getCode(), badSize, 0x55, 0x55, 0x55, 0x55, 0x55};
    ...
    void equalsAndHashCodeTest() {
        ...
        byte[] good = new byte[]{0x00, SerializationMechanism.JAVA.getCode(), 0x00, 0x55, 0x55, 0x55, 0x55, 0x55};
        ...
        byte[] good2 = new byte[]{0x00, SerializationMechanism.JAVA.getCode(), 0x00, 0x55, 0x55, 0x56, 0x57, 0x58};
        dp2 = new DataPacket(good2);
        assertFalse(dp1.equals(dp2));
        assertFalse(dp2.equals(dp1));
        assertNotEquals(dp1.hashCode(), dp2.hashCode());

        byte[] good_short = new byte[]{0x00, SerializationMechanism.JAVA.getCode(), 0x00, 0x55, 0x55, 0x58};
